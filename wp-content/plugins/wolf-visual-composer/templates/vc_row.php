...
if ( class_exists( 'Wolf_Vc_Content_Block' ) ) {
	...
	if ( preg_match( '/\[wvc_content_block [a-zA-Z0-9-_=" ]+\]/', $content, $match ) ) {
		if ( isset( $match[0] ) ) {
...
if ( 'true' === $add_bigtext || 'yes' === $add_bigtext ) {
	...
	$background_html .= "<div class='wvc-row-bigtext-container wvc-row-big-text-vertical-align-$bt_vertical_align'>";
...
if ( $add_effect ) {
	$background_html .= apply_filters( 'wvc_background_effect', '' );
}
