	...
	if ( $do_object_fit ) {
		...
		if ( wp_attachment_is_image( $background_img ) ) {
			...
			$output .= '<img
				...
				class="' . wvc_sanitize_html_classes( $cover_class ) . '"
...
function wvc_parallax_video_bg( $args ) {
	...
	if ( $video_bg_start_time ) {
		$data_start_time .= 'data-jarallax-video-start-time="' .  absint( $video_bg_start_time ) . '"';
	}
	...
	$output .= '<div class="wvc-video-parallax" data-jarallax-video="' . esc_attr( $video_bg_url ) . '" ' . $data_start_time . '>';
